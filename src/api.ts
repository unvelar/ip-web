...
const SIMULATED_LOGIN_MODE_KEY = "simulated_login_mode";
const SIMULATED_LOGIN_TOKEN_KEY = "simulated_login_token";
const SIMULATED_LOGIN_ACTING_TENANT_KEY = "simulated_login_acting_tenant";
let simulatedLoginAuthMode =
  new URLSearchParams(window.location.search).get("simulated_login") === "1" ||
  sessionStorage.getItem(SIMULATED_LOGIN_MODE_KEY) === "1";
if (simulatedLoginAuthMode) {
  sessionStorage.setItem(SIMULATED_LOGIN_MODE_KEY, "1");
}
let token: string | null = simulatedLoginAuthMode
  ? sessionStorage.getItem(SIMULATED_LOGIN_TOKEN_KEY)
...
export function setToken(t: string | null) {
  token = t;
  if (simulatedLoginAuthMode) {
    if (t) sessionStorage.setItem(SIMULATED_LOGIN_TOKEN_KEY, t);
    else sessionStorage.removeItem(SIMULATED_LOGIN_TOKEN_KEY);
    return;
  }
...
export function setSimulatedLoginToken(t: string) {
  simulatedLoginAuthMode = true;
  sessionStorage.setItem(SIMULATED_LOGIN_MODE_KEY, "1");
  token = t;
  sessionStorage.setItem(SIMULATED_LOGIN_TOKEN_KEY, t);
}
...
let actingTenant: string | null = simulatedLoginAuthMode
  ? sessionStorage.getItem(SIMULATED_LOGIN_ACTING_TENANT_KEY)
  : localStorage.getItem("acting_tenant");
...
export function setActingTenant(t: string | null) {
  actingTenant = t;
  if (simulatedLoginAuthMode) {
    if (t) sessionStorage.setItem(SIMULATED_LOGIN_ACTING_TENANT_KEY, t);
    else sessionStorage.removeItem(SIMULATED_LOGIN_ACTING_TENANT_KEY);
    return;
  }
