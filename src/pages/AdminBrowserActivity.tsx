...
export default function AdminBrowserActivity() {
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      try {const result=await getWorkers(view==="fleet"?fleetQuery:"",view==="fleet"?fleetState:"",view==="fleet"?workerBefore:undefined,controller.signal);if(!controller.signal.aborted){setWorkers(result);setWorkerError(null);setWorkerLoadedKey(workerKey);}}
  ...
  return <AdminPage section="browser-activity" title="Browser activity" description="A live view of the pages, captures, and outcomes across your browser workers." wide
    ...
    <div className="ba-stats admin-card" aria-label="Browser worker counts">
      <div><span>Browser workers</span><strong>{workers?total.toLocaleString():"…"}</strong><small>{workers?.counts.starting?`${workers.counts.starting} starting · `:""}Fleet status every 15 seconds</small></div>
    ...
    </main><aside className="ba-rail"><div className="ba-rail-title"><h2>Workers now</h2><button onClick={()=>{setView("fleet");setFleetQuery("");setFleetState("");setWorkerBefore(undefined);setWorkerPages([]);}}>View fleet<ChevronRight size={13}/></button></div>
      {workerError?<div className="ba-muted" role="alert">Worker status is unavailable. <button onClick={()=>setWorkerRevision(value=>value+1)}>Retry</button></div>:!workers?<p className="ba-muted">Loading workers…</p>:workers.workers.slice(0,8).map(item=><WorkerTile key={item.id} worker={item} selected={item.id===worker} onClick={()=>chooseWorker(item.id)}/>)}
