...
import { Clock3, Cpu, GitBranch, Server } from "lucide-react";
...
export default function AdminCompute() {
  ...
  return (
    <AdminPage section="compute" title="Compute" description="Manage GPU capacity, hardware profiles, and job routing.">
      <section className="admin-card admin-card-padded">
        <div className="admin-capacity-row">
          <div className="admin-capacity-copy flex items-start gap-3">
        ...
        <div className="mt-5 border-t border-stone-100 pt-5">
          <div className="min-w-0">
            ...
            <div className="admin-scaling-fields">
              <label className="grid gap-1 text-xs font-semibold text-stone-600">
                Maximum pods
                <input
                  type="number"
          ...
          <>
            ...
            <div className="p-5">
              ...
              <div className="overflow-x-auto rounded-xl border border-stone-200">
                <table className="admin-routing-table min-w-full divide-y divide-stone-200 text-left">
                  <thead className="bg-stone-50 text-[10px] font-bold uppercase tracking-wide text-stone-500">
                    <tr>
                      <th className="admin-route-target px-3 py-2.5">Job</th>
                      <th className="admin-route-target px-3 py-2.5">Execution class</th>
                      <th className="admin-route-action px-3 py-2.5 text-right">Apply</th>
                    </tr>
                  </thead>
                    ...
                    {routes.map((route) => {
                      ...
                      return (
                        <tr key={route.job_type}>
                          <td className="admin-route-job px-3 py-2.5 text-xs font-semibold text-stone-800">
                            {formatJobType(route.job_type)}
                          </td>
                          <td className="admin-route-target px-3 py-2.5">
                            <select
                              value={selected}
                              aria-label={`Execution class for ${formatJobType(route.job_type)}`}
                              onChange={(event) => setRouteDrafts((current) => ({
                                ...current,
                                [route.job_type]: event.target.value,
                              }))}
                              className="h-9 min-w-48 rounded-lg border border-stone-200 bg-white px-2.5 text-xs font-medium text-stone-800 focus:border-red-600 focus:outline-none focus:ring-2 focus:ring-red-500/10"
                            >
                              {profiles.map((profile) => (
                          ...
                          <td className="admin-route-action px-3 py-2.5 text-right">
                            <button
                              type="button"
