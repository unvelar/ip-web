...
export function WorkerTypeBadge({ kind, compact = false }: { kind: ExecutionKind; compact?: boolean }) {
  ...
  return (
    <span title={copy.detail} aria-label={`${["scrapfly", "scrapedo"].includes(kind) ? "Execution" : "Worker type"}: ${copy.label}`} className={`inline-flex shrink-0 items-center gap-1 rounded border ${compact ? "px-1" : "px-1.5"} py-0.5 text-[9px] font-semibold ${copy.style}`}>
