...
export function reconcileProductCommercialSubgroupCount<
  T extends {
    triage_member_count: number | null;
    commercial_subgroups: Array<ProductCommercialSubgroupFields>;
  },
>(group: T, subgroupKey: string, exactCount: number): T {
  const boundedCount = Math.max(0, Math.trunc(exactCount));
  const subgroup = group.commercial_subgroups.find(({ key }) => key === subgroupKey);
  if (!subgroup || subgroup.triage_member_count === boundedCount) return group;
  const delta = boundedCount - subgroup.triage_member_count;
  return {
    ...group,
    triage_member_count: Math.max(0, (group.triage_member_count ?? 0) + delta),
    commercial_subgroups: group.commercial_subgroups.map((candidate) =>
      candidate.key === subgroupKey
        ? { ...candidate, triage_member_count: boundedCount }
        : candidate
    ),
  };
}
