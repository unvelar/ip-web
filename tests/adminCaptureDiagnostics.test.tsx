...
const getAttempts = mock(async () => ({ attempts: [
...
mock.module("../src/api", () => ({ getAdminCaptureAttempts: getAttempts }));
...
let root: Root;
afterEach(() => { if (root) act(() => root.unmount()); document.body.replaceChildren(); getAttempts.mockClear(); });
...
test("expansion fetches distinct execution records even when attempt numbers repeat", async () => {
  ...
  expect(getAttempts).toHaveBeenCalledWith("job", undefined);
